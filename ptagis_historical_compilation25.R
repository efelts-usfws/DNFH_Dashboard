...
marking.dat_25 <- read_csv("historical/DNFH Marking Historical 2025.csv") |> 
...
release_sites_25 <- marking.dat_25 |> 
...
release_grp_tally_25 <- marking.dat_25 |> 
...
marking.join_25 <- marking.dat_25 |> 
  left_join(release_grp_tally_25,
            by=c("hatchery","species","release_sitecode",
                 "release_date"))

marking.export_25 <- marking.join_25 |> 
...
saveRDS(marking.export_25,
        "historical/mark_2025")
...
for(i in 1:length(tag.split)){
  ...
  write.table(tag.split[i],file=str_c("historical/int_files_25/",
...
int.files <- list.files("historical/complete_history_25", pattern="\\.csv$", full.names = T)
...
int_25.dat <- map(int.files,read_csv) |> 
  bind_rows() |> 
